...
float sensor_diff_roll=0;
float sensor_accum_roll=0;


float current_pitch=0;
float previous_pitch=0;
float sensor_diff_pitch=0;
float sensor_accum_pitch=0;


float current_yaw=0;
float previous_yaw=0;
float previous_yaw1=0;
float sensor_diff_yaw=0;
float sensor_accum_yaw=0;
...
//  MadgwickQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f,  my,  mx, mz);
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
          ...
          if(SerialDebug)
          {
            ...
            Serial.print("sensor_accum__roll = "); Serial.println(sensor_accum_roll);
            Serial.print("sensor_accum_pitch = "); Serial.println(sensor_accum_pitch);
            Serial.print("sensor_accum__yaw = "); Serial.println(sensor_accum_yaw);
          }
...
void RollPitchYaw_accumulation(void)
{
   const uint32_t a=0;
   const uint32_t b=180000;
  ...
  current_roll=roll; 
  sensor_diff_roll=current_roll-previous_roll;
  sensor_accum_roll += sensor_diff_roll;
  if(a<millis() && millis()<b)
  {
    sensor_accum_roll=0; 
  }
  ...
  current_pitch=pitch; 
  sensor_diff_pitch=current_pitch-previous_pitch;
  sensor_accum_pitch += sensor_diff_pitch;
  if(a<millis() && millis()<b)
  {
    sensor_accum_pitch=0; 
  }
  previous_pitch=current_pitch; 
    
  current_yaw=yaw; 
  sensor_diff_yaw=current_yaw-previous_yaw;
  sensor_accum_yaw += sensor_diff_yaw;
  if(a<millis() && millis()<b)
  {
    sensor_accum_yaw=0; 
  }
